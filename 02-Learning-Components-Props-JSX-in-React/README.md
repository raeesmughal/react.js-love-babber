## Learning Components/Props/JSX in React 2024


### components

1. first clean the folder structure
2. create component folder
3. create card component
4. export this component and import and render (multiple cards) in App.jsx
5. In these cards, the name, image and descirption is static but you can change them by passing props.


### props

defination : props help you create your component dynamic.

1. pass the props (its simple) where you are rendering your component
2. in your .jsx file for component receive these props as parameter for your component function and use in your component structure.
3. 


